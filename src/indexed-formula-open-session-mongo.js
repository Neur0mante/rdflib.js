    ...
    return MongoClient.connect(url)
      .then((db) => {
        ...
        return Promise.all([
          ...
          // ),
          // Subject is a prefix for the compound index so it doesn't need a dedicated initiation
          // statements.createIndex(
          //   { subject: 1 },
          ...
          statements.createIndex(
            ...
            { unique: true },
            { partialFilterExpression: { $and: [ { subject: { $exists: true } }, { predicate: { $exists: true } }, { object: { $exists: true } } ] } }
          ),
          statements.createIndex(
            { predicate: 1, object: 1 },
            { partialFilterExpression: { $and: [ { predicate: { $exists: true } }, { object: { $exists: true } } ] } }
          )
        ])
          .then(() => {
            // db.close()
    ...
    return MongoClient.connect(url)
      .then((db) => {
        ...
        return statements.deleteMany(
          this.buildQuery(subject, predicate, object, why))
          .then(() => {
            db.close()
          })
      })
